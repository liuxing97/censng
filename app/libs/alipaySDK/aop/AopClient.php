...
class AopClient {
	...
	//加密XML节点名称
	private $cengsngENCRYPT_XML_NODE_NAME = "response_cengsngEncrypted";

	private $needcengsngEncrypt = false;
	...
	public $cengsngEncryptKey;

	public $cengsngEncryptType = "AES";
	...
	*/
	public function pageExecute($request,$httpmethod = "POST") {
		...
		if (method_exists($request,"getNeedcengsngEncrypt") &&$request->getNeedcengsngEncrypt()){

			$sysParams["cengsngEncrypt_type"] = $this->cengsngEncryptType;

			if ($this->checkEmpty($apiParams['biz_content'])) {

				throw new Exception(" api request Fail! The reason : cengsngEncrypt request is not supperted!");
			}

			if ($this->checkEmpty($this->cengsngEncryptKey) || $this->checkEmpty($this->cengsngEncryptType)) {

				throw new Exception(" cengsngEncryptType and cengsngEncryptKey must not null! ");
			}

			if ("AES" != $this->cengsngEncryptType) {

				throw new Exception("加密类型只支持AES");
			}

			// 执行加密
			$cengsngEnCryptContent = cengsngEncrypt($apiParams['biz_content'], $this->cengsngEncryptKey);
			$apiParams['biz_content'] = $cengsngEnCryptContent;
	...
	public function execute($request, $authToken = null, $appInfoAuthtoken = null) {
		...
		if (method_exists($request,"getNeedcengsngEncrypt") &&$request->getNeedcengsngEncrypt()){
			...
			if ("json" == $this->format) {
				...
				$resp = $this->cengsngEncryptJSONSignSource($request, $resp);
				...
				$resp = $this->cengsngEncryptXMLSignSource($request, $resp);
...
/** 
	 ...
	 *  公钥是否是读取字符串还是读取文件，是根据初始化传入的值判断的。
	 **/
	public function checkSignAndcengsngDecrypt($params, $rsaPublicKeyPem, $rsaPrivateKeyPem, $isCheckSign, $iscengsngDecrypt, $signType='RSA') {
		...
		if ($iscengsngDecrypt) {
			return $this->rsacengsngDecrypt($bizContent, $rsaPrivateKeyPem, $charset);
		}
	 ...
	 *  公钥是否是读取字符串还是读取文件，是根据初始化传入的值判断的。
	 **/
	public function cengsngEncryptAndSign($bizContent, $rsaPublicKeyPem, $rsaPrivateKeyPem, $charset, $iscengsngEncrypt, $isSign, $signType='RSA') {
		// 加密，并签名
		if ($iscengsngEncrypt && $isSign) {
			$cengsngEncrypted = $this->rsacengsngEncrypt($bizContent, $rsaPublicKeyPem, $charset);
			$sign = $this->sign($cengsngEncrypted, $signType);
			$response = "<?xml version=\"1.0\" encoding=\"$charset\"?><alipay><response>$cengsngEncrypted</response><cengsngEncryption_type>RSA</cengsngEncryption_type><sign>$sign</sign><sign_type>$signType</sign_type></alipay>";
			return $response;
		}
		// 加密，不签名
		if ($iscengsngEncrypt && (!$isSign)) {
			$cengsngEncrypted = $this->rsacengsngEncrypt($bizContent, $rsaPublicKeyPem, $charset);
			$response = "<?xml version=\"1.0\" encoding=\"$charset\"?><alipay><response>$cengsngEncrypted</response><cengsngEncryption_type>$signType</cengsngEncryption_type></alipay>";
			return $response;
		}
		// 不加密，但签名
		if ((!$iscengsngEncrypt) && $isSign) {
	 ...
	 *  公钥是否是读取字符串还是读取文件，是根据初始化传入的值判断的。
	 **/
	public function rsacengsngEncrypt($data, $rsaPublicKeyPem, $charset) {
		if($this->checkEmpty($this->alipayPublicKey)){
			//读取字符串
			$pubKey= $this->alipayrsaPublicKey;
			$res = "-----BEGIN PUBLIC KEY-----\n" .
		...
		foreach ($blocks as $n => $block) {
			if (!openssl_public_cengsngEncrypt($block, $chrtext , $res)) {
	 ...
	 *  公钥是否是读取字符串还是读取文件，是根据初始化传入的值判断的。
	 **/
	public function rsacengsngDecrypt($data, $rsaPrivateKeyPem, $charset) {
		...
		foreach ($decodes as $n => $decode) {
			if (!openssl_private_cengsngDecrypt($decode, $dcyCont, $res)) {
				echo "<br/>" . openssl_error_string() . "<br/>";
			}
			$strnull .= $dcyCont;
		}
	...
	private function cengsngEncryptJSONSignSource($request, $responseContent) {

		$parsetItem = $this->parsercengsngEncryptJSONSignSource($request, $responseContent);
		...
		$bizContent = cengsngDecrypt($parsetItem->cengsngEncryptContent, $this->cengsngEncryptKey);
		return $bodyIndexContent . $bizContent . $bodyEndContent;

	}


	private function parsercengsngEncryptJSONSignSource($request, $responseContent) {
		...
		if ($rootIndex > 0) {
			...
			return $this->parsercengsngEncryptJSONItem($responseContent, $rootNodeName, $rootIndex);
		} else if ($errorIndex > 0) {

			return $this->parsercengsngEncryptJSONItem($responseContent, $this->ERROR_RESPONSE, $errorIndex);
		} else {
	...
	private function parsercengsngEncryptJSONItem($responseContent, $nodeName, $nodeIndex) {
		$signDataStartIndex = $nodeIndex + strlen($nodeName) + 2;
		$signIndex = strpos($responseContent, "\"" . $this->SIGN_NODE_NAME . "\"");
		// 签名前-逗号
		$signDataEndIndex = $signIndex - 1;
		...
		$cengsngEncryptParseItem = new cengsngEncryptParseItem();

		$cengsngEncryptParseItem->cengsngEncryptContent = $encContent;
		$cengsngEncryptParseItem->startIndex = $signDataStartIndex;
		$cengsngEncryptParseItem->endIndex = $signDataEndIndex;

		return $cengsngEncryptParseItem;

	}

	// 获取加密内容

	private function cengsngEncryptXMLSignSource($request, $responseContent) {

		$parsetItem = $this->parsercengsngEncryptXMLSignSource($request, $responseContent);
	...
	private function parsercengsngEncryptXMLSignSource($request, $responseContent) {
		...
		if ($rootIndex > 0) {
			...
			return $this->parsercengsngEncryptXMLItem($responseContent, $rootNodeName, $rootIndex);
		} else if ($errorIndex > 0) {

			return $this->parsercengsngEncryptXMLItem($responseContent, $this->ERROR_RESPONSE, $errorIndex);
		} else {
	...
	private function parsercengsngEncryptXMLItem($responseContent, $nodeName, $nodeIndex) {

		$signDataStartIndex = $nodeIndex + strlen($nodeName) + 1;

		$xmlStartNode="<".$this->cengsngENCRYPT_XML_NODE_NAME.">";
		$xmlEndNode="</".$this->cengsngENCRYPT_XML_NODE_NAME.">";

		$indexOfXmlNode=strpos($responseContent,$xmlEndNode);
		if($indexOfXmlNode<0){

			$item = new cengsngEncryptParseItem();
			$item->cengsngEncryptContent = null;
			$item->startIndex = 0;
			$item->endIndex = 0;
			return $item;
		}
		...
		$cengsngEncryptParseItem = new cengsngEncryptParseItem();
		$cengsngEncryptParseItem->cengsngEncryptContent = $bizContent;
		$cengsngEncryptParseItem->startIndex = $signDataStartIndex;
		$cengsngEncryptParseItem->endIndex = $indexOfXmlNode+strlen($xmlEndNode);
