...
@section("pageMain")
    ...
    <div class="templateItems">
        ...
        <div class="templateItemList">
            ...
            @endphp
            @foreach($templateWebData as $templateWebList)
            {{--一个模板--}}
            <div class="item">
                {{--展示--}}
                <div class="show">
                    <img class="showImg{{$templateWebList['number']}} showImg" src="/templatesImg/{{$templateWebList['number']}}.jpg" />
                </div>
                {{--动态查看--}}
                <div class="hovering">
                    {{--标题--}}
                    <div class="temItemTitle">{{$templateWebList['title']}}<br><span style="font-size: 16px">模板编号-{{$templateWebList['number']}}</span></div>
                    {{--放大镜--}}
                    <div onclick="$('body').append(zoomFileNumber('{{$templateWebList['number']}}'));$('html,body').addClass('ovfHiden')" class="chakan"><span class="magnifierIcon"></span></div>
                    {{--购买--}}
                    <a href="/template/buy/{{$templateWebList['number']}}">
                        <div class="xuanze">
